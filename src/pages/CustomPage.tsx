...
import { Layers, X, RotateCcw, Search, ChevronDown, ChevronRight } from 'lucide-react'
...
export default function CustomPage() {
  ...
  const { lang } = ctx
  const { items, setItems, addItem, removeItem, reset, loaded } = useCustomLayout()
  ...
  // Convert items to Layout format (what RGL needs)
  ...
  function onLayoutChange(next: Layout) {
    // onLayoutChange only updates positions — it never removes items.
    // Items are only removed via removeItem(). This prevents a race condition
    // where RGL fires onLayoutChange with an intermediate empty layout during
    // external drop, which would overwrite the item just added by addItem().
    ...
    // If merged has fewer items than our state, RGL layout is in a transient
    // state (e.g., placeholder removed, new item not yet reflected). Skip.
    if (merged.length < items.length) return
    // Avoid redundant saves if nothing changed
  ...
  // Called when user drops an external palette item onto the grid
  ...
  return (
    <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start', minHeight: 'calc(100vh - 250px)' }}>
      {/* CSS for RGL + palette interactions */}
      <style>{`
        ...
        .custom-grid .react-grid-item.cssTransforms {
        ...
        .custom-grid .react-grid-item.resizing {
          opacity: 0.85;
          z-index: 10;
        }
        .palette-item {
        ...
        .palette-item[data-dragging="true"] {
          opacity: 0.5;
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            ...
            <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>
              {pt ? 'Paleta de componentes' : 'Component palette'}
            </div>
          </div>
          <div style={{ position: 'relative' }}>
            ...
            <input
              type="text"
              placeholder={pt ? 'Buscar componente…' : 'Search component…'}
              value={query}
                    ...
                    {catItems.map(item => {
                      ...
                      return (
                        <div
                          ...
                          title={pt
                            ? `Arraste para o canvas à direita. Tamanho padrão: ${item.defaultW}×${item.defaultH}.`
    ...
    <div style={{
      ...
      <div>
        ...
        <div style={{ fontSize: 12, color: 'var(--text-tertiary)', lineHeight: 1.6, maxWidth: 440 }}>
          {pt
            ? 'Arraste componentes da paleta à esquerda para esta área. Depois, posicione e redimensione do jeito que quiser — tudo é salvo automaticamente.'
            : 'Drag components from the palette on the left onto this area. Then position and resize them however you want — everything saves automatically.'}
        </div>
      </div>
    </div>
  )
