...
A member can push to more than one central (`preferences.team.connections: TeamConnection[]`) and can block specific git repositories **per connection** — a repo hidden from central A can still go to central B. The full design is `docs/superpowers/specs/2026-07-28-multi-central-and-repo-sharing-design.md`; this is what shipped from it. Three layers:

- **The pure decision layer — `server/share-rules.ts`.** `sessionShared(session, denied, index)` is the one predicate that decides whether a session may leave the machine for a given connection: a session is denied when its own `git_remote` (or the remote learned for its `project_path` via `buildPathRepoIndex`, for adapters that never stamp one) is in the connection's canonicalized denylist, or when its directory is known to hold more than one repo and *any* of them is denied — a workspace holding a shared and a blocked repo would otherwise leak the blocked one's `first_prompt` under the shared key. `filterShared` / `filterSharedWorkflows` apply it to sessions and workflow runs; a run whose owning session is unknown is dropped, never kept. `buildSplitStatsCache` is the attribution-split half — see below. Every function is pure and fails closed: uncertain attribution means "not shared", never "shared by default".
...
**The split cache is selected by the declared rule, `hasRestrictions(conn.deniedRepos)` — never by comparing filtered vs. unfiltered counts.** A count-based switch fails open on a cold consolidate store (`0 < 0` is false) and flips without any corresponding user action. On the restricted path there is **no fallback to the unsplit cache, ever**: `buildSplitStatsCache` returns `null` whenever it cannot build a faithful split (no attribution boundary, an unreadable real cache, the cold-store signature), and the push omits `statsCache` entirely rather than shipping the unfiltered one.
