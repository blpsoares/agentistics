...
export default function HomePage() {
  ...
  function renderCard(id: CardId) {
    ...
    if (id === 'messages') {
      ...
      card = <StatCard lang={lang} label={lang === 'pt' ? 'Tokens entrada' : 'Input tokens'} value={full ? fmtFull(totalInputTokens) : fmt(totalInputTokens)} sub={lang === 'pt' ? 'entrada NOVA — sem o cache' : 'FRESH input — cache not included'} icon={<Download size={15} />} accent="var(--accent-blue)" info={infoItems[8]} onInfoClick={() => setInfoModalIndex(8)} fullPrecision={full} onTogglePrecision={tog(totalInputTokens)} />
    } else if (id === 'output-tokens') {
      card = <StatCard lang={lang} label={lang === 'pt' ? 'Tokens saída' : 'Output tokens'} value={full ? fmtFull(totalOutputTokens) : fmt(totalOutputTokens)} sub={lang === 'pt' ? 'gerados pelo modelo, com raciocínio' : 'generated by the model, thinking included'} icon={<Upload size={15} />} accent="var(--accent-purple)" info={infoItems[9]} onInfoClick={() => setInfoModalIndex(9)} fullPrecision={full} onTogglePrecision={tog(totalOutputTokens)} />
    } else if (id === 'cost') {
