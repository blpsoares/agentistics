...
const HELP_EN: ContentSection[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      { text: 'Screens: Services (start/stop/restart this machine, a central or the Docker machine; turn boot on or off; and the SETUP wizard — solo / central / member, the history-preservation consent, connecting to or leaving a central — reached from the config pane\'s mode row), Sessions (the fleet running right now), Dashboard (the metrics — the whole of `agentop tui`, on 1-5 or tab, `f` filters by assistant), Logs, Cheat sheet, Help, Contribute.' },
  ...
  },
  {
    ...
    // A machine reading `▤ 4/18` in the header above `5 on screen · 29 known` in the list looked
    ...
    rows: [
      ...
      { cmd: '▤ n/m', text: 'The parallel-sessions budget: assistant processes alive, out of how many this machine has the MEMORY to hold. It is not a session count — `m` grows and shrinks with free RAM, and turns red as you approach it.' },
...
const HELP_PT: ContentSection[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      { cmd: '▤ n/m', text: 'O orçamento de sessões em paralelo: processos de assistente vivos, sobre quantos esta máquina tem MEMÓRIA para segurar. Não é uma contagem de sessões — o `m` sobe e desce com a RAM livre, e fica vermelho quando você se aproxima dele.' },
