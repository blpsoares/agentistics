...
const ROOT = join(SERVER_PKG, '..', '..')
const distDir = join(ROOT, 'packages', 'web', 'dist')
const outputFile = join(SERVER_PKG, 'server', 'embedded-dist.generated.ts')
...
console.log(`Embedded ${count} assets (${kb} KB) → packages/server/server/embedded-dist.generated.ts`)
